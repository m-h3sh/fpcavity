...
# Defining variables
wavelength = 1064e-9
cav_len = 0.5 # resonant length for wavelength 1064 nm
...
# w0 = np.sqrt((3e8/laser.f) * ((93.15 + 26)*1e-1 / 2) / np.pi)
# glaser = Gauss("glaser", laser.p1.o, w0=w0, z=0)
# model.add(glaser)

# Defining the mirrors

mx1 = Mirror("mx1", R=0.999, L=0.0, Rc=-3*cav_len, xbeta=0.0, ybeta=0.0)
mx2 = Mirror("mx2", R=0.999, L=0.0, Rc=-3*cav_len, xbeta=0.0, ybeta=0.0)
my1 = Mirror("my1", R=0.999, L=0.0, xbeta=0.0, ybeta=0.0)
my2 = Mirror("my2", R=0.999, L=0.0, xbeta=0.0, ybeta=0.0)
model.add(mx1)
model.add(mx2)
model.add(my1)
model.add(my2)
...
pd_trans = PowerDetector("pdt", mx2.p2.o)
model.add(pd_trans)
...
xaxis = Xaxis(mx1.xbeta, 'lin', -20, 20, 400)
...
plt.plot(output["pdt"], label="Transmitted Power")
